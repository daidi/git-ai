...
const TRANSLATIONS: Record<string, Record<string, string>> = {
    en: {
        ...
        'actions.btn.clean': 'Clean Stuck Commits',
        ...
        'cmd.clean.progress': 'Git AI: Cleaning stuck commits...',
        'cmd.clean.success': '✅ Git AI: Commits cleaned successfully!\n\n{0}',
        'cmd.clean.failed': 'Git AI clean failed: {0}',
        'cmd.clean.pushedWarning': 'Some target commits have already been pushed. Editing them will rewrite Git history and requires force pushing. Proceed?',
        'cmd.clean.forceYes': 'Force Clean & Rewrite History',
        'cmd.clean.cancel': 'Cancel',
    ...
    'zh-cn': {
        ...
        'cmd.clean.progress': 'Git AI: 正在清理卡住的提交记录...',
        'cmd.clean.success': '✅ Git AI: 提交清理成功！\n\n{0}',
        'cmd.clean.failed': 'Git AI 清理失败: {0}',
        'cmd.clean.pushedWarning': '检测到部分目标 Commit 已被 push 到远端。修改它们将重写 Git 历史，并且后续需要强制推送（Force Push）。是否仍要继续？',
        'cmd.clean.forceYes': '确认修改并重写历史',
        'cmd.clean.cancel': '取消',
    ...
    'zh-tw': {
        ...
        'actions.section.actions': '操作', 'actions.section.tools': '工具', 'actions.btn.retry': '重試 AI 潤飾', 'actions.btn.undo': '復原', 'actions.btn.cancel': '終止目前的潤飾', 'actions.btn.forcePush': '馬上推播到遠端', 'actions.btn.showLogs': '日誌', 'actions.btn.config': '設定', 'actions.btn.reinit': '重新初始化', 'actions.btn.skipNext': '略過 AI (下一次)', 'actions.btn.clean': '清理卡住的提交',
    ...
    'ja': {
        ...
        'actions.section.actions': 'アクション', 'actions.section.tools': 'ツール', 'actions.btn.retry': 'AI推敲を再試行', 'actions.btn.undo': '元に戻す', 'actions.btn.cancel': '推敲をキャンセル', 'actions.btn.forcePush': '今すぐ強制プッシュ', 'actions.btn.showLogs': 'ログを表示', 'actions.btn.config': '設定', 'actions.btn.reinit': '再初期化', 'actions.btn.skipNext': 'AIをスキップ(次回)', 'actions.btn.clean': 'スタックしたコミットをクリーンアップ',
    ...
    'ko': {
        ...
        'actions.section.actions': '작업', 'actions.section.tools': '도구', 'actions.btn.retry': '재시도', 'actions.btn.undo': '실행 취소', 'actions.btn.cancel': '취소', 'actions.btn.forcePush': '강제 푸시', 'actions.btn.showLogs': '로그 보기', 'actions.btn.config': '설정', 'actions.btn.reinit': '재초기화', 'actions.btn.skipNext': 'AI 건너뛰기', 'actions.btn.clean': '중단된 커밋 정리',
    ...
    'pt': {
        ...
        'actions.section.actions': 'Ações', 'actions.section.tools': 'Ferramentas', 'actions.btn.retry': 'Repetir', 'actions.btn.undo': 'Desfazer', 'actions.btn.cancel': 'Cancelar', 'actions.btn.forcePush': 'Forçar Push', 'actions.btn.showLogs': 'Logs', 'actions.btn.config': 'Opções', 'actions.btn.reinit': 'Inicializar', 'actions.btn.skipNext': 'Pular', 'actions.btn.clean': 'Limpar Commits Travados',
    ...
    'vi': {
        ...
        'actions.section.actions': 'Thao tác', 'actions.section.tools': 'Công cụ', 'actions.btn.retry': 'Thử lại', 'actions.btn.undo': 'Hoàn tác', 'actions.btn.cancel': 'Hủy', 'actions.btn.forcePush': 'Ép Đẩy(Push)', 'actions.btn.showLogs': 'Xem Log', 'actions.btn.config': 'Cài đặt', 'actions.btn.reinit': 'Khởi tạo', 'actions.btn.skipNext': 'Bỏ qua lần tới', 'actions.btn.clean': 'Dọn dẹp commit bị kẹt',
    ...
    'id': {
        ...
        'actions.section.actions': 'Aksi', 'actions.section.tools': 'Alat', 'actions.btn.retry': 'Coba lagi', 'actions.btn.undo': 'Urungkan AI', 'actions.btn.cancel': 'Batalkan AI', 'actions.btn.forcePush': 'Buru Push', 'actions.btn.showLogs': 'Munculkan Log', 'actions.btn.config': 'Setelan', 'actions.btn.reinit': 'Setel Ulang', 'actions.btn.skipNext': 'Lewati aja', 'actions.btn.clean': 'Bersihkan Commit Macet',
